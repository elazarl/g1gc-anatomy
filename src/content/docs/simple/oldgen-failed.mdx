---
title: 'Tenuring Objects to the Old Gen'
...
public class MakeOldgen {
  ...
  public static void main(String[] args) {
    ...
    long MB = 1024*1024;
    int tenuring_thres =
       GetVmOptions.get("MaxTenuringThreshold");
  ...
  private static Eldery makeOldgen(int mb) {
    Eldery oldgen = null;
    // each Eldery object has 512 bytes, we'll allocate 2*1024
    // of those to take up 1 MB
    for (int i = 0; i < mb * 1024 * 2; i++) {
      Eldery newold = new Eldery();
      newold.next = oldgen;
      oldgen = newold;
    }
    return oldgen;
